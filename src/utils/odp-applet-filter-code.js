...
if (
  CityOfEdmontonHenryStaging.openData.DataSet ==
  'LRT Elevator/Escalator Outages'
) {
  const columnValues = JSON.parse(
    CityOfEdmontonHenryStaging.openData.ColumnValues
  ).sort(sortNumber)
  const outages = JSON.parse(CityOfEdmontonHenryStaging.openData.AllValues)
    .map((entry: any) => {
      return ` ${entry.lrt_station_name}: ${entry.lrt_device_location} `
    })
    .sort()
